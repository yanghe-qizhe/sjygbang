...
namespace EOS.WebApp.Areas.CommonModule.Controllers
{
    ...
    public class PermissionController : Controller
    {
        ...
        /// <param name="Category">对象分类:1-部门2-角色3-岗位4-群组</param>
        /// <returns></returns>
        public ActionResult ModuleTreeSCJ(string ObjectId, string Category)
        {
            DataTable dt = base_modulepermissionbll.GetListSCJ(ObjectId, Category);
            List<TreeJsonEntity> TreeList = new List<TreeJsonEntity>();
            if (DataHelper.IsExistRows(dt))
            {
                foreach (DataRow item in dt.Rows)
        ...
        /// </summary>
        /// <param name="ObjectId">对象主键</param>
        /// <param name="Category">对象分类:1-部门2-角色3-岗位4-群组5-仓库</param>
        /// <returns></returns>
        public ActionResult StoreList(string ObjectId, string Category)
        {
            StringBuilder sb = new StringBuilder();
            DataTable dt = base_modulepermissionbll.GetStoreList(ObjectId, Category);
            foreach (DataRow item in dt.Rows)
            {
                string Genderimg = "car.png";
                string strchecked = "";
                if (!string.IsNullOrEmpty(item["objectid"].ToString()))//判断是否选中
                {
                    strchecked = "selected";
                }
                sb.Append("<li class=\"" + item["pk_org"] + " " + strchecked + "\">");
                sb.Append("<a class=\"a_" + strchecked + "\" id=\"" + item["id"] + "\" title='编号："+ item["code"] + "\r\n名称：" + item["name"] + "'><img src=\"/Content/Images/Icon16/" + Genderimg + "\">" + item["name"] + "</a><i></i>");
                sb.Append("</li>");
            }
        ...
        public ActionResult AuthorizedSubmit(string ModuleId, string ModuleButtonId, string ViewDetailId, string ObjectId, string Category)
        {
            ...
            try
            {
                ...
                StringBuilder sbDelete_Module = new StringBuilder("delete from Base_ModulePermission where ObjectId =" + DbHelper.DbParmChar + "ObjectId AND Category=" + DbHelper.DbParmChar + "Category");
                ...
                StringBuilder sbDelete_Button = new StringBuilder("delete from Base_ButtonPermission where ObjectId = " + DbHelper.DbParmChar + "ObjectId AND Category=" + DbHelper.DbParmChar + "Category");
                ...
                StringBuilder sbDelete_View = new StringBuilder("delete from Base_ViewPermission where ObjectId = " + DbHelper.DbParmChar + "ObjectId AND Category=" + DbHelper.DbParmChar + "Category");
        ...
        #region 分配供应商
        [ManagerPermission(PermissionMode.Enforce)]
        public ActionResult AllotSupply()
        {
            return View();
        }
        /// <summary>
        ...
        /// <param name="Category">对象分类:1-部门2-角色3-岗位4-群组</param>
        /// <returns></returns>
        public ActionResult SupplyList(string ObjectId, string Category)
        {
            string ModuleId = "999eae8b-1d09-4678-9b5f-2901205f1d98";
            StringBuilder sb = new StringBuilder();
            DataTable ListData = base_datascopepermissionbll.GetScopeSupplyList(ModuleId, ObjectId, Category);
            if (ListData != null && ListData.Rows.Count != 0)
            {
                foreach (DataRow item in ListData.Rows)
                {
                    ...
                    sb.Append("<li class=\"" + item["id"] + " " + strchecked + "\">");
                    sb.Append("<a class=\"a_" + strchecked + "\" id=\"" + item["id"] + "\" title='名称：" + item["NAME"] + "'>" + item["SHORTNAME"] + "</a><i></i>");
                    sb.Append("</li>");
                }
            }
            return Content(sb.ToString());
        ...
        [LoginAuthorize]
        public ActionResult GroupListJson(string ObjectId, string Category, JqGridParam jqgridparam)
        {
            try
            {
                Stopwatch watch = CommonHelper.TimerStart();
                DataTable dt = base_datascopepermissionbll.GetScopeGroupList(ObjectId, Category, jqgridparam);
                var JsonData = new
                {
        ...
        [LoginAuthorize]
        public ActionResult SupplyListJson(string ObjectId, string Category, JqGridParam jqgridparam)
        {
            try
            {
                Stopwatch watch = CommonHelper.TimerStart();
                DataTable dt = base_datascopepermissionbll.GetScopeSupplyList(ObjectId, Category, jqgridparam);
                var JsonData = new
                {
                    total = jqgridparam.total,
                    page = jqgridparam.page,
        ...
        [LoginAuthorize]
        public ActionResult CustomerListJson(string ObjectId, string Category, JqGridParam jqgridparam)
        {
            try
            {
                Stopwatch watch = CommonHelper.TimerStart();
                DataTable dt = base_datascopepermissionbll.GetScopeCustomerList(ObjectId, Category, jqgridparam);
                var JsonData = new
                {
                    total = jqgridparam.total,
                    page = jqgridparam.page,
        ...
        [LoginAuthorize]
        public ActionResult TrafficListJson(string ObjectId, string Category, JqGridParam jqgridparam)
        {
            try
            {
                Stopwatch watch = CommonHelper.TimerStart();
                DataTable dt = base_datascopepermissionbll.GetScopeTrafficList(ObjectId, Category, jqgridparam);
                var JsonData = new
                {
                    total = jqgridparam.total,
                    page = jqgridparam.page,
        ...
        [ManagerPermission(PermissionMode.Enforce)]
        public ActionResult AllotMemberIndex()
        {
            return View();
        }
        /// <summary>
        ...
        /// <returns></returns>
        public ActionResult AuthorizedMember(string UserId, string ObjectId, string CompanyId, string Category)
        {
            try
            {
                string[] array = UserId.Split(',');
                int IsOk = base_objectuserrelationbll.BatchAddMember(array, ObjectId, CompanyId, Category);
        ...
        public ActionResult AuthorizedMember1(string UserId, string ObjectId, string Category)
        {
            try
            {
                string[] array = UserId.Split(',');
                int IsOk = base_objectuserrelationbll.BatchAddMember1(array, ObjectId, Category);
                return Content(new JsonMessage { Success = true, Code = IsOk.ToString(), Message = "操作成功。" }.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult ScopeCompanyList(string ObjectId, string Category, string ModuleId)
        {
        ...
        #region 物料分类
        public ActionResult ScopeMARBASClassList(string ObjectId, string Category, string ModuleId)
        {
           // string ModuleId = "2028a926-9deb-4025-ac61-ed94fcc74065";
            DataTable DataList = base_datascopepermissionbll.ScopeMARBASClassList(ModuleId, ObjectId, Category);
            List<TreeJsonEntity> TreeList = new List<TreeJsonEntity>();
            if (DataHelper.IsExistRows(DataList))
            {
                foreach (DataRow item in DataList.Rows)
                {
                    bool hasChildren = false;
                    DataTable childnode = DataHelper.GetNewDataTable(DataList, "PARENTID='" + item["ID"].ToString() + "'");
                    if (childnode.Rows.Count > 0)
                    {
                        hasChildren = true;
                    }
        ...
        public ActionResult ScopeWeightNoList(string ObjectId, string Category, string ModuleId)
        {
            //string ModuleId = "737e1995-e673-4da2-99e2-2b6c5e98c97c";
            DataTable DataList = base_datascopepermissionbll.ScopeWeightNoList(ModuleId, ObjectId, Category);
            List<TreeJsonEntity> TreeList = new List<TreeJsonEntity>();
            if (DataHelper.IsExistRows(DataList))
            {
                foreach (DataRow row in DataList.Rows)
                {
                    TreeJsonEntity tree = new TreeJsonEntity();
                    string PostId = row["code"].ToString();
                    bool hasChildren = false;
                    tree.checkstate = row["objectid"].ToString() != "" ? 1 : 0;
                    tree.showcheck = true;
                    tree.id = PostId;
