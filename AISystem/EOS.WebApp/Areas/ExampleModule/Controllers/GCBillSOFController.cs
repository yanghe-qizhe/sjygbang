...
{
    public class GCBillSOFController : PublicController<DP_GCCARSORDER>
    {
        string ModuleId = "";

        DP_GCCARSORDERBll orderbll = new DP_GCCARSORDERBll();
        ...
        /// <returns></returns>
        [LoginAuthorize]
        public ActionResult GCBillSOFForm()
        {
        ...
        //列表
        [LoginAuthorize]
        public ActionResult GCBillSOFIndex()
        {
            return View();
        }
        //明细
        [LoginAuthorize]
        public ActionResult GCBillSOFDetail()
        {
            return View();
        }
        /// <summary>
        ...
        /// <returns></returns>
        [LoginAuthorize]
        public ActionResult GCBillSOFQuery()
        {
            return View();
        }
        ...
        public ActionResult GetOrderList(string BillNo, string StartTime, string EndTime, JqGridParam jqgridparam)
        {
            try
            {
                ...
                List<VDP_GCCARSORDER> ListData = orderbll.GetOrderList(BillNo, StartTime, EndTime, jqgridparam);
                var JsonData = new
                {
        ...
        /// 订单列表（返回Json）
        [LoginAuthorize]
        public ActionResult GetQYOrderList(string BillNo, string Material, string Cars, string ISUSE, string StartTime, string EndTime, JqGridParam jqgridparam)
        {
            try
            {
                Stopwatch watch = CommonHelper.TimerStart();

                List<VDP_GCCARSORDER> ListData = orderbll.GetOrderList(BillNo, Material, Cars, ISUSE, StartTime, EndTime, jqgridparam);
                var JsonData = new
                {
                    total = jqgridparam.total,
                    page = jqgridparam.page,
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult SubmitOrderForm(string KeyValue, DP_GCCARSORDER entity, string ModuleId)
        {
            #region 验证
            //车辆验证
            ...
            //身份证验证
            jsonmsg = CHECK_CARDNUMBER_ORDER(entity.PSDNO, 2, entity.ID,"");
            if (jsonmsg != null)
            {
                return Content(jsonmsg.ToString());
            }
            ...
            try
            {
                ...
                {
                    string sql = String.Format("select count(1) from DP_GCCARSORDER where  ID='{0}'", entity.ID);
                    ...
                    ADD_CARSORDER(entity.CARS, entity.CARSNAME, entity.CARDNO, entity.DRIVERS, entity.DRIVERSNAME, entity.PSDNO, "", "", entity.ID, 6, 0, database, isOpenTrans);
        ...
        {
            //验证
            string sql = String.Format("select count(1) from DP_GCCARSORDER where ID='{0}' and isuse!=4", KeyValue);
            int res = DataFactory.Database().FindCountBySql(sql);
            if (res > 0)
            {
            ...
            {
                string Message = "删除成功。";
                database.Delete("DP_GCCARSORDER", "ID", KeyValue);
        ...
        {
            //验证
            DP_GCCARSORDER entity = repositoryfactory.Repository().FindEntity(KeyValue);
            if (entity.ISUSE == "4")
            {
