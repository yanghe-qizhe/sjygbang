...
{
    public class QRBillSOFController : PublicController<DP_OTCARSORDER>
    {
        string ModuleId = "";
        ...
        /// <returns></returns>
        [LoginAuthorize]
        public ActionResult QRBillSOFForm()
        {
        ...
        //列表
        [LoginAuthorize]
        public ActionResult QRBillSOFIndex()
        {
            return View();
        }
        //明细
        [LoginAuthorize]
        public ActionResult QRBillSOFDetail()
        {
            return View();
        }
        /// <summary>
        ...
        /// <returns></returns>
        [LoginAuthorize]
        public ActionResult QRBillSOFQuery()
        {
            return View();
        }
        ...
        /// 订单列表（返回Json）
        [LoginAuthorize]
        public ActionResult GetOrderList(string BillNo,string PK_ORG, string Supply, string Material, string Cars, string ISUSE, string StartTime, string EndTime, JqGridParam jqgridparam)
        {
            try
            {
                Stopwatch watch = CommonHelper.TimerStart();

                List<VDP_OTCARSORDER> ListData = orderbll.GetCGOrderList(BillNo, PK_ORG, Supply, Material, Cars, ISUSE, "2", StartTime, EndTime, jqgridparam);
                var JsonData = new
                {
        ...
        public ActionResult SubmitOrderForm(string KeyValue, DP_OTCARSORDER entity, string ModuleId)
        {
            #region 验证
            //车辆验证
            JsonMessage jsonmsg = CHECK_CARS_ORDER(entity.CARS, 3, entity.ID,"");
            if (jsonmsg != null)
            {
                return Content(jsonmsg.ToString());
            }
            //身份证验证
            jsonmsg = CHECK_CARDNUMBER_ORDER(entity.PSDNO, 3, entity.ID,"");
            if (jsonmsg != null)
            {
                return Content(jsonmsg.ToString());
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    ADD_CARSORDER(entity.CARS, entity.CARSNAME, entity.CARDNO, entity.DRIVERS, entity.DRIVERSNAME, entity.PSDNO, "", "", entity.ID, 3, 0, database, isOpenTrans);
