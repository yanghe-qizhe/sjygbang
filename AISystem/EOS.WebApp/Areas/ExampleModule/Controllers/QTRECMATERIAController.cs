...
namespace EOS.WebApp.Areas.ExampleModule.Controllers
{
    ...
    /// 其他销售发货
    /// </summary>
    public class QTRECMATERIAController : PublicController<APP_HANDORDER>
    {
        string ModuleId = "";
        ...
        /// <returns></returns>
        [LoginAuthorize]
        public ActionResult QTBillRECFForm()//POBillCOFForm
        {
        ...
        //列表页面
        [LoginAuthorize]
        public ActionResult QTBillRECFIndex()
        {
            return View();
        }
        ...
        //明细页面
        [LoginAuthorize]
        public ActionResult QTBillRECFDetail()
        {
            return View();
        }
        /// <summary>
        ...
        /// <returns></returns>
        [LoginAuthorize]
        public ActionResult QTBillRECFQuery()
        {
            return View();
        }
        ...
        /// <returns></returns>
        [LoginAuthorize]
        public ActionResult QTSELECTBillRECF()
        {
            return View();
        }
        ...
        public ActionResult GetOrderList(string TaskNo, string User, string StartTime, string EndTime, JqGridParam jqgridparam)
        {
            try
            {
                ...
                List<VAPP_HANDORDERQT> ListData = vorderbll.GetOrderList(TaskNo, User, StartTime, EndTime, "4", "1", jqgridparam);
                var JsonData = new
                {
        ...
        public ActionResult GetSeOrderList(string TaskNo, string Material, string Store, string Cars, string Supply, string User, string StartTime, string EndTime, JqGridParam jqgridparam)
        {
            try
            {
                Stopwatch watch = CommonHelper.TimerStart();
                VAPP_HANDORDERQTBll vorderbll = new VAPP_HANDORDERQTBll();
                List<VAPP_HANDORDERQT> ListData = vorderbll.GetRKOrderList(TaskNo, Supply, Material, Store, Cars, User, "4", "1", StartTime, EndTime, jqgridparam);
                var JsonData = new
                {
                    total = jqgridparam.total,
                    page = jqgridparam.page,
        ...
        /// <returns></returns>
        [LoginAuthorize]
        public ActionResult GetWbOrderList(string BILLNO, string Supply, string Material, string Cars, string finish, string StartTime, string EndTime, string ZFFLAG, JqGridParam jqgridparam)
        {
            try
            {
                Stopwatch watch = CommonHelper.TimerStart();
                VWB_WEIGHTBSBll orderbll = new VWB_WEIGHTBSBll();
                List<VWB_WEIGHTBS> ListData = orderbll.GetQTRKOrderList(BILLNO, "", "", "", Supply, Material, Cars, "", "4", finish, ZFFLAG, StartTime, EndTime, jqgridparam);
                var JsonData = new
                {
                    total = jqgridparam.total,
                    page = jqgridparam.page,
        ...
        public ActionResult SubmitOrderForm(APP_HANDORDER entity, string KeyValue, string ModuleId)
        {
            ...
            try
            {
                ...
                if (res > 0)
                {
                    return Content(new JsonMessage { Success = false, Code = "-1", Message = "请勿重复收货登记：" + entity.BILLNO }.ToString());
                }
                if (!string.IsNullOrEmpty(KeyValue))
                {
        ...
        public ActionResult QuerenOrderForm(APP_HANDORDER entity, string KeyValue, string ModuleId)
        {
            ...
            try
            {
                ...
                string Message = KeyValue == "" ? "收货成功。" : "收货成功。";
                ...
                if (res > 0)
                {
                    return Content(new JsonMessage { Success = false, Code = "-1", Message = "已走退货流程：" + entity.BILLNO + "不允许收货" }.ToString());
                }
                if (!string.IsNullOrEmpty(KeyValue))
                {
                    entity.Modify(KeyValue);
        ...
        public ActionResult GetEntity(string KeyValue)
        {
            ...
            string strWhere = string.Format(" WHERE FINISH='0' and TYPE='{0}' and ICCARD='{1}'", 4, KeyValue);
