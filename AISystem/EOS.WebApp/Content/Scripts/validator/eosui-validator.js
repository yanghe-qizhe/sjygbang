...
//验证不为零 或者不为null,空
function isDoubleOrZero(obj) {
    var controlObj = $.trim(obj);
    if (controlObj.length == 0 || controlObj == null || controlObj == undefined) {
        return false;
    }
    if (obj.length != 0) {
        if (obj == "0.00" || obj == "0" || obj == "0.000") {
            return false;
        } else {
            return true;
        }
...
function isExists(pars) {
    var bol = false;
    getAjax('/Utility/IsExists', pars, function (rs) {
...
function JudgeValidate(obj) {
    ...
    $(obj).find("[datacol=yes]").each(function () {
        if ($(this).attr("checkexpession") != undefined) {
            ...
            var arr = $(this).attr("checkexpession").split(',');
            for (var i = 0; i < arr.length; i++) {
                switch (arr[i]) {
                    ...
                    case "DoubleOrZero":
                        {
                            if (!isDoubleOrZero($(this).attr("value"))) {
                                Validatemsg = $(this).attr("err") + "不能为零！\n";
                                Validateflag = false;
                                tipCss($(this), Validatemsg); return false;
                            }
                            break;
                    ...
                    case "IsExist":
                        {
                            ...
                            if (!isNotNull($(this).attr("sumlength"))) {
                                if (!isLenghtNum($(this).attr("value"), $(this).attr("sumlength"))) {
                                    Validatemsg = $(this).attr("err") + "必须为" + $(this).attr("sumlength") + "位数字！\n";
                                    Validateflag = false;
                                    tipCss($(this), Validatemsg); return false;
                                }
                            }
                            ...
                            var pars = 'table=' + table + "&value=" + value + "&field=" + field + "&key=" + key;
                            var id = $("#" + $(this).attr("idval")).val();
                            if (id != "" || id != undefined) {
                                pars += "&id=" + id + "&_" + new Date().getTime();
                            }
                            if (!isExists(pars)) {
