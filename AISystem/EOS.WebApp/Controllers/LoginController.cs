...
namespace EOS.WebApp.Controllers
{
    ...
    public class LoginController : Controller
    {
        ...
        /// 调试日志
        /// </summary>
        public static EOS.Utilities.LogHelper log = EOS.Utilities.LogFactory.GetLogger("LoginController");
            ...
            try
            {
                ...
                if (Account == ConfigHelper.AppSettings("CurrentUserName"))
                {
                    if (ConfigHelper.AppSettings("CurrentPassword") == Password)
                    {
                        ...
                        imanageuser.CompanyName = "系统";
                        imanageuser.DepartmentId = "系统";
                        imanageuser.DepartmentName = "系统";
                        imanageuser.DepartmentPId = "系统";
                        imanageuser.DepartmentPName = "系统";
                        imanageuser.IPAddress = IPAddress;
                        imanageuser.IPAddressName = IPAddressName;
                        imanageuser.IsSystem = true;
                        imanageuser.ZCUser =1;
                        imanageuser.IsCheckData = 0;
                ...
                else
                {
                    ...
                    switch (outmsg)
                    {
                        ...
                        case "succeed": //验证成功
                            ...
                            VBase_Department dept = DataFactory.Database().FindEntity<VBase_Department>(base_user.DepartmentId);
                            imanageuser.DepartmentName = dept.FullName;
                            imanageuser.DepartmentPId = dept.ParentId;
                            imanageuser.DepartmentPName = dept.ParentName;

                            imanageuser.ObjectId = base_objectuserrelationbll.GetObjectId(imanageuser.UserId);
                            imanageuser.IPAddress = IPAddress;
                            imanageuser.IPAddressName = IPAddressName;
                            imanageuser.IsSystem = false;
                            imanageuser.IsCheckData = base_user.ISCHECKDATA;
                            imanageuser.ZCUser = base_user.ZCUser;
                            imanageuser.InnerUser = base_user.InnerUser;
                            ManageProvider.Provider.AddCurrent(imanageuser);
                            //对在线人数全局变量进行加1处理
                            HttpContext rq = System.Web.HttpContext.Current;
                            rq.Application["OnLineCount"] = (int)rq.Application["OnLineCount"] + 1;
        ...
        public ActionResult CheckUserLogin(string Account, string Password, string Token)
        {
            ...
            try
            {
                ...
                {
                    Base_User base_user = base_userbll.CheckUserLogin(Account, Password, out outmsg);
                    switch (outmsg)
                    {
                        case "-1":      //账户不存在
                            Msg = "-1";
                        ...
                        case "gluser":   //不是管理员
                            Msg = "5";
                            Base_SysLogBll.Instance.WriteLog(Account, OperationType.Login, "-1", "账户不是管理员、IP所在城市：" + IPAddressName);
                            break;

                        default:
                            break;
                    }
                    msg.BillNo = base_user.RealName;
                }
                msg.Success = true;
                msg.Code = "1";
                msg.Message = Msg;
        ...
        public ActionResult OutLogin()
        {
            if (ManageProvider.Provider.Current().IsSystem == false)
            {
               ...
               // Base_User entity = new Base_User();
                //entity = base_userbll.SetFormControl(UserId);
                //entity.Onlines = 0;
                //base_userbll.Repository().Update(entity);
                System.Text.StringBuilder strSql = new System.Text.StringBuilder();
                strSql.Append(string.Format("update Base_User set Onlines=0 where USERID='{0}'", UserId));
